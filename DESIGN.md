...
```
video-automation/
├── assets/                    # 静的アセット（フォント、BGM、効果音、背景動画）
│   ├── background_videos/    # 背景動画ライブラリ
│   │   ├── opening/          # オープニング用動画
│   │   ├── main/             # メイン用動画
│   │   └── ending/           # エンディング用動画
│   ├── bgm/                  # BGMライブラリ
...
│   │   └── urban/            # 都市伝説ジャンル用BGM
│   ├── fonts/                # フォントファイル
│   │   └── cinema/           # 字幕用フォント（cinecaption226.ttf）
│   └── sfx/                  # 効果音
│       └── impact_title.mp3  # セクションタイトル用効果音
│
├── config/                   # 設定ファイル
│   ├── genres/               # ジャンル別設定
│   │   ├── ijin.yaml         # 偉人ジャンル設定
│   │   └── urban.yaml        # 都市伝説ジャンル設定
│   ├── phases/               # フェーズ別設定
│   │   ├── audio_generation.yaml
│   │   ├── subtitle_generation.yaml
│   │   ├── video_composition.yaml
│   │   └── ...
│   ├── prompts/              # プロンプトテンプレート（Jinja2）
│   │   ├── script/           # 台本生成用プロンプト
│   │   ├── image/            # 画像生成用プロンプト
│   │   └── thumbnail/        # サムネイル生成用プロンプト
│   └── variations/           # バリエーション設定
│
├── data/                     # データディレクトリ
│   ├── input/                # 入力データ（手動台本YAML）
│   ├── working/              # 作業ディレクトリ（各フェーズの出力）
│   │   └── {subject}/        # 偉人ごとの作業ディレクトリ
│   │       ├── 01_script/    # Phase 1: 台本
│   │       ├── 02_audio/     # Phase 2: 音声
│   │       ├── 03_images/    # Phase 3: 画像
│   │       ├── 06_subtitles/ # Phase 6: 字幕
│   │       ├── 07_composition/ # Phase 7: 動画統合
│   │       └── 08_thumbnail/ # Phase 8: サムネイル
│   └── output/               # 最終出力
│       ├── videos/           # 完成動画
│       └── thumbnails/       # サムネイル
│
...
│   ├── core/                 # コア機能
│   ├── phases/               # フェーズ実装
│   ├── generators/           # 生成器（画像、音声、字幕など）
│   ├── processors/           # データ処理ユーティリティ
│   └── utils/                # 汎用ユーティリティ
│
├── scripts/                  # 補助スクリプト
│   ├── convert_manual_script.py  # 手動台本YAML→JSON変換
│   ├── create_script_template.py # 台本テンプレート生成
│   └── ...
│
└── docs/                     # ドキュメント
    ├── ARCHITECTURE.md       # アーキテクチャ説明
    ├── COMMON_COMMANDS.md    # よく使うコマンド
    └── ...
```
...
| ファイル | 役割 |
|---------|------|
| `config_manager.py` | 設定ファイル（YAML）の読み込み・管理 |
| `phase_base.py` | 全フェーズの基底クラス（共通処理） |
| `orchestrator.py` | 全フェーズの順次実行を管理 |
| `models.py` | Pydanticモデル定義（データ構造） |
| `exceptions.py` | カスタム例外クラス |
| `database.py` | データベース操作（未使用） |

**重要なクラス**:
- `PhaseBase`: 全フェーズの基底クラス
  - `execute_phase()`: フェーズ実行の抽象メソッド
  - `validate_output()`: 出力検証
  - `check_inputs_exist()`: 入力ファイル確認
- `ConfigManager`: 設定ファイル管理
  - `get_phase_config(phase_num)`: フェーズ設定取得
  - `get_genre_config(genre)`: ジャンル設定取得
- `PhaseOrchestrator`: フェーズ実行管理
  - `run_all_phases()`: 全フェーズ実行
...
| ファイル | フェーズ | 役割 |
|---------|---------|------|
| `phase_01_script.py` | Phase 1 | 台本生成（Claude API） |
| `phase_01_auto_script.py` | Phase 1 | 自動台本生成（未使用） |
| `phase_02_audio.py` | Phase 2 | 音声生成（Kokoro/ElevenLabs） |
| `phase_03_images.py` | Phase 3 | AI画像生成（Stable Diffusion） |
| `phase_04_animation.py` | Phase 4 | 画像アニメーション（無効化） |
| `phase_05_bgm.py` | Phase 5 | BGM選択（無効化、Phase 7で統合） |
| `phase_06_subtitles.py` | Phase 6 | 字幕生成（Legacy版） |
| `phase_06_subtitles_v2.py` | Phase 6 | 字幕生成（V2版、推奨） |
| `phase_07_composition.py` | Phase 7 | 動画統合（FFmpeg版） |
| `phase_07_composition_v2.py` | Phase 7 | 動画統合（V2版、推奨） |
| `phase_07_composition_legacy.py` | Phase 7 | 動画統合（MoviePy版、Legacy） |
| `phase_07_composition_legacy02.py` | Phase 7 | 動画統合（MoviePy版、Legacy02） |
| `phase_08_thumbnail.py` | Phase 8 | サムネイル生成 |
| `phase_09_youtube.py` | Phase 9 | YouTubeアップロード |
| `phase_10_shorts.py` | Phase 10 | ショート動画生成 |
| `phase_11_tiktok.py` | Phase 11 | TikTokアップロード |

**重要な実装**:
- **Phase 6 V2**: `special_type`を`audio_timing.json`から読み取り、セクションタイトルを検出
- **Phase 7 V2**: 背景動画 + 画像 + 字幕 + BGM + 効果音を統合
  - セクションタイトル演出（黒オーバーレイ、効果音、BGM音量調整）
...
| カテゴリ | ファイル | 役割 |
|---------|---------|------|
| **音声生成** | `audio_generator.py` | 音声生成器のファクトリー |
| | `kokoro_audio_generator.py` | Kokoro TTS音声生成 |
| | `azure_audio_generator.py` | Azure Speech音声生成 |
| **画像生成** | `image_generator.py` | 画像生成器のファクトリー |
| | `stable_diffusion_generator.py` | Stable Diffusion画像生成 |
| | `image_collector.py` | 画像収集（未使用） |
| **字幕生成** | `subtitle_generator/` | 字幕生成モジュール |
| | `subtitle_generator/generator.py` | 字幕生成のメインロジック |
| | `subtitle_generator/timing_processor.py` | タイミング処理 |
| | `subtitle_generator/text_splitter.py` | テキスト分割 |
| | `subtitle_generator/formatter.py` | フォーマット変換 |
| **台本生成** | `script_generator.py` | 台本生成（Claude API） |
| | `script_prompt_builder.py` | プロンプト構築 |
| **サムネイル生成** | `thumbnail_generator.py` | サムネイル生成器のファクトリー |
| | `intellectual_curiosity_generator.py` | 知的欲求サムネイル生成 |
| | `universal_thumbnail_generator.py` | 汎用サムネイル生成 |
| | `pillow_thumbnail_generator.py` | Pillowベースのサムネイル生成 |
| **背景動画** | `background_video_selector.py` | 背景動画選択ロジック |

**重要な実装**:
- `subtitle_generator/generator.py`: 文字レベルタイミングから字幕を生成
- `kokoro_audio_generator.py`: ElevenLabs FA統合、Whisperフォールバック
- `intellectual_curiosity_generator.py`: SD画像生成 + テキストオーバーレイ
...
| カテゴリ | ファイル | 役割 |
|---------|---------|------|
| **字幕処理** | `subtitle_utils/ass_generator.py` | ASS字幕ファイル生成 |
| | `subtitle_utils/style_loader.py` | 字幕スタイル設定読み込み |
| | `subtitle_utils/style_converter.py` | スタイル設定変換 |
| | `subtitle_utils/animation_tags.py` | アニメーションタグ生成 |
| **動画統合** | `video_composition/bgm_processor.py` | BGM処理（ループ、フェード、音量調整） |
| | `video_composition/ffmpeg_builder.py` | FFmpegコマンド構築 |
| | `video_composition/background_processor.py` | 背景動画処理 |
| | `video_composition/image_processor.py` | 画像処理 |
| **タイミング** | `whisper_timing.py` | Whisperタイミング抽出 |
| | `elevenlabs_forced_alignment.py` | ElevenLabs FAタイミング抽出 |
| | `image_timing_matcher.py` | 画像タイミングマッチング |
| | `image_timing_matcher_fixed.py` | 画像タイミングマッチング（修正版） |
| | `image_timing_matcher_llm.py` | LLM駆動画像タイミングマッチング |
| **その他** | `logger.py` | ログ設定 |
| | `image_resizer.py` | 画像リサイズ（1920x1080変換） |
| | `youtube_metadata_generator.py` | YouTubeメタデータ生成 |
| | `tiktok_uploader.py` | TikTokアップロード |

**重要な実装**:
- `video_composition/bgm_processor.py`: BGMフィルター生成、効果音統合、タイトル区間の音量調整
- `video_composition/ffmpeg_builder.py`: FFmpegコマンド構築（ASS字幕、BGM、効果音対応）
- `subtitle_utils/ass_generator.py`: ASS字幕生成（SectionTitleスタイル対応）

---

#### `src/processors/` - データ処理

| ファイル | 役割 |
|---------|------|
| `audio_processor.py` | 音声処理（結合、無音挿入） |
| `text_optimizer.py` | テキスト最適化 |
| `bgm_manager.py` | BGM管理（未使用、Phase 7で統合） |
| `image_animator.py` | 画像アニメーション（未使用） |
| `video_compositor.py` | 動画合成（未使用、Phase 7で統合） |
...
|------------|------|
| `phases/` | 各フェーズの処理方法設定 |
| `genres/` | ジャンル別設定（プロンプト、BGMパスなど） |
| `prompts/` | Jinja2プロンプトテンプレート |
| `variations/` | バリエーション設定（選択肢） |

**重要な設定ファイル**:
- `phases/video_composition.yaml`: Phase 7の設定
  - `section_title`: セクションタイトル演出設定
  - `bgm`: BGM音量・フェード設定
  - `split_layout`: 二分割レイアウト設定
- `phases/subtitle_generation.yaml`: Phase 6の設定
  - `styles`: 字幕スタイル（none, normal, section_title）
  - `timing`: タイミング設定
- `phases/audio_generation.yaml`: Phase 2の設定
  - `use_elevenlabs_fa`: ElevenLabs FA使用フラグ
  - `punctuation_pause`: 句点での間隔制御
...
Phase 2: 音声生成
  ...
  audio_timing.json (文字レベルタイミング)
    ↓
Phase 3: 画像生成
  section_XX_sd_*.png (1920x1080)
  classified.json
    ↓
Phase 6: 字幕生成
  subtitles.srt
  subtitle_timing.json (special_type付き)
    ↓
Phase 7: 動画統合
  {subject}_final.mp4
    ↓
Phase 8: サムネイル生成
  thumbnails/*.png (1280x720)
    ↓
...
**`src/phases/phase_06_subtitles_v2.py`**

**主要メソッド**:
- `execute_phase()`: 字幕生成のメイン処理
- `_set_special_type_from_audio_timing()`: `audio_timing.json`から`special_type`を設定
- `_save_timing_json()`: `subtitle_timing.json`を保存（`special_type`含む）

**処理フロー**:
1. `audio_timing.json`を読み込み
2. 字幕生成器で字幕を生成
3. `audio_timing.json`の`title_timing`から`special_type`を設定
4. `subtitle_timing.json`に保存

---

#### Phase 7: 動画統合（V2版）

**`src/phases/phase_07_composition_v2.py`**

**主要メソッド**:
- `execute_phase()`: 動画統合のメイン処理
- `_detect_section_title_segments()`: セクションタイトル区間を検出
- `_create_video_with_background()`: 背景動画 + 画像 + 字幕を統合
- `_create_bgm_filter_for_background()`: BGMフィルター生成（効果音・タイトル区間対応）

**処理フロー**:
1. データ読み込み（音声、画像、字幕、BGM）
2. セクションタイトル区間を検出
3. 背景動画を処理
4. 画像concatファイル作成
5. BGMフィルター生成（効果音・タイトル区間の音量調整含む）
6. FFmpegで動画生成（Pass 1: 背景+画像+音声）
7. ASS字幕を焼き込み（Pass 2: 字幕追加）

---

#### BGM処理

**`src/utils/video_composition/bgm_processor.py`**

**主要メソッド**:
- `create_bgm_filter_for_background()`: BGMフィルター生成
  - セクションごとのBGMループ・トリミング
  - フェードイン/アウト
  - タイトル区間での音量調整（`bgm_volume_multiplier`）
  - 効果音の統合
- `get_audio_duration()`: 音声ファイルの長さ取得

**処理内容**:
- BGMをループして音声の長さに合わせる
- セクションごとにBGMを切り替え
- タイトル区間でBGM音量を下げる（デフォルト70%）
- 効果音をタイトル開始時に再生
...
**`src/utils/video_composition/ffmpeg_builder.py`**

**主要メソッド**:
- `build_ffmpeg_command_with_ass()`: ASS字幕を使用したFFmpegコマンド構築
- `build_ffmpeg_command_optimized()`: 最適化版FFmpegコマンド構築
...
**`src/utils/subtitle_utils/ass_generator.py`**

**主要メソッド**:
- `create_ass_header()`: ASSヘッダー生成（スタイル定義含む）
- `create_ass_file()`: ASSファイル全体を生成

**スタイル**:
- `Default`: 通常字幕（60px、下部配置）
- `SectionTitle`: セクションタイトル（120px、中央配置、太字）

---

### データモデル（`src/core/models.py`）

**重要なモデル**:
- `SubtitleEntry`: 字幕エントリ
  - `special_type`: `"section_title"`でタイトル字幕を識別
- `VideoComposition`: 動画統合結果
- `SubtitleGeneration`: 字幕生成結果
- `AudioGeneration`: 音声生成結果
...
**Phase 6 → Phase 7 のデータフロー**:

```
Phase 2: audio_timing.json
  {
    "title_timing": {
    ...
    ↓
Phase 6: subtitle_timing.json
  {
    "subtitles": [{
      "special_type": "section_title",  ← Phase 6で設定
      "start_time": 0.0,
      "end_time": 2.639
    }]
  }
...
Phase 7: タイトル区間検出
  - 黒オーバーレイ（不透明度90%）
  - 効果音（impact_title.mp3）
  - BGM音量調整（70%）
  - SectionTitleスタイルで字幕表示（120px、中央）
```
