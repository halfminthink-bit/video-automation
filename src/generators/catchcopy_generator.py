...
視聴者の注目を引き、クリック率を最大化する**超短文・超インパクト**の文言を生成してください。

【重要な原則】
1. **最大7文字以内**（理想は5文字）- 絶対厳守
2. **感情に強く訴える**
3. **数字があれば必ず使う**（99%, 3秒など）
4. **対立・驚き・謎・命令を演出**
5. **体言止め・命令形を積極的に使う**
6. **説明的な文章は絶対にNG**
...
【悪い例】
- 「99%が知らない」（8文字） - 長すぎる
- 「医師を敵にした」（7文字） - ギリギリだが説明的
- 「命を救った手の秘密」（10文字超） - 長すぎる・説明的

【感情タイプ】
各キャッチコピーに感情タイプを付与してください：
- dramatic: 劇的・感動的（例:「死を止めた」）
- command: 命令形（例:「手を洗え！」）
- mystery: 謎・不思議（例:「99%失敗」）
- fact: 事実・数字（例:「3秒で変わる」）
- contrast: 対立・衝突（例:「医師vs医師」）
...
- **メインタイトル: {main_length}文字以内（絶対厳守）**
- サブタイトル: {sub_length}文字以内（オプション・不要ならnull）

【重要な指示】
1. メインタイトルは**必ず{main_length}文字以内**（理想は5文字）
2. **数字、対立、驚き、謎、命令形**を積極的に使う
3. **台本の核心的な要素を1つに絞る**
4. **感情を直接刺激する言葉**を選ぶ
5. 各キャッチコピーに**感情タイプ**を付与（dramatic/command/mystery/fact/contrast）

以下のJSON形式で{num_candidates}つの候補を出力してください：
{{
  "candidates": [
    {{
      "main_title": "5-7文字の超インパクトタイトル",
      "sub_title": null,
      "emotion": "dramatic",
...
【各候補のアプローチ例】
1. 命令形+対立（「手を洗え！」emotion: command）
2. 数字+驚き（「99%失敗」emotion: mystery）
3. 対立構造（「医師vs医師」emotion: contrast）
4. 劇的+謎（「死を止めた」emotion: dramatic）
5. 時間+事実（「3秒変化」emotion: fact）

注意: 文字数を絶対に超えないこと！7文字を1文字でも超えたら失敗です！"""
