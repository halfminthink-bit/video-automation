...
- **メインタイトル: {main_length}文字以内（厳守）**
- サブタイトル: {sub_length}文字以内（オプション）

【重要な指示】
1. メインタイトルは**必ず{main_length}文字以内**にしてください
2. **数字、対立、驚き、謎、命令形**を積極的に使う
3. **台本の核心的な要素を1つに絞る**（あれもこれも盛り込まない）
4. **感情を直接刺激する言葉**を選ぶ
5. サブタイトルは無理に作らなくても良い（nullでOK）

以下のJSON形式で{num_candidates}つの候補を出力してください：
{{
  "candidates": [
    {{
      "main_title": "5-10文字の超インパクトタイトル",
      "sub_title": "サブタイトル（なければnull）",
