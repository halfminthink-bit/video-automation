...
Purpose: YouTube thumbnail background - must grab attention immediately while leaving space for text overlay."""
    ...
    def _load_font(self, size: int) -> ImageFont.FreeTypeFont:
        ...
        """
        # フォントパスのリスト（優先順位順）
        # プロジェクト内フォント -> Windows -> Linux -> Macの順で探索
        font_paths = [
            # プロジェクト内のフォント（最優先）
            "assets/fonts/GenEiKiwamiGothic-EB.ttf",
            "assets/fonts/NotoSansJP-Bold.ttf",
            "assets/fonts/SourceHanSansJP-Bold.otf",
            # Windows
        ...
        # フォールバック: エラーを発生させる
        self.logger.error("No Japanese font found! Please check font availability.")
        self.logger.error("Project fonts: assets/fonts/GenEiKiwamiGothic-EB.ttf or NotoSansJP-Bold.ttf")
        ...
        raise FileNotFoundError(
            "No Japanese font found. Please check assets/fonts/ or install system fonts."
        )
