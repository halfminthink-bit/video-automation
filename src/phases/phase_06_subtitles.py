...
from src.utils.subtitle_timing_optimizer import SubtitleTimingOptimizer
...
class Phase06Subtitles(PhaseBase):
    ...
    def execute_phase(self) -> SubtitleGeneration:
        ...
        try:
            ...
            subtitles = timing_optimizer.adjust_subtitle_timing_with_sentence_end(subtitles)

            # 3.5. 全字幕の一律延長は不要（句点延長で制御するため無効化）
            # self.logger.info("Extending subtitle display times (0.2s margin before next)...")
            # subtitles = timing_optimizer.extend_subtitle_display(subtitles)
